...
function ign_menu( $item, $args ) {
	...
	if ( in_array( 'menu-item-has-children', (array) $classes ) ) {
		$item .= '<button tabindex="-1" class="submenu-dropdown-toggle">' . $arrow . '
                    <span class="screen-reader-text">' . __( 'Expand child menu', 'digivate-test' ) . '</span></button>';
	}
 ...
 * @return void
 */
function digivate_test_output_inline_svg( $html ) {
...
add_filter( 'get_custom_logo', 'digivate_test_output_inline_svg' );
 ...
 *
 */
function digivate_test_resource_hints( $urls, $relation_type ) {
	if ( wp_style_is( 'digivate-test-fonts', 'queue' ) && 'preconnect' === $relation_type ) {
		$urls[] = array(
...
add_filter( 'wp_resource_hints', 'digivate_test_resource_hints', 10, 2 );
